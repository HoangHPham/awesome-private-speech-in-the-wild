# 📌 Awesome Speech Privacy in the Wild

A curated collection of resources on **privacy-preserving speech technology in real-world conditions**, including noisy environments, overlapping speakers, and multi-device scenarios.

This repository aims to support research in robust and trustworthy speech systems that preserve user privacy without sacrificing performance.

---

## 📚 Contents

- [Surveys](#-surveys)
- [Key Papers](#-key-papers)
- [Datasets](#-datasets)
- [Sub-topics](#-sub-topics)
  - Speaker Representation
  - Speaker Disentanglement
  - Speaker Anonymization
  - Diffusion Models
  - Distributed Learning
- [Benchmarks](#-benchmarks)
- [Open-source Toolkits](#-open-source-toolkits)
- [Challenges & Competitions](#-challenges--competitions)
- [Contribution](#-contribution)
- [License](#-license)

---

## 📖 Surveys

A collection of survey papers covering speech privacy, robustness, and speaker modeling.

- [Privacy in Speech Technology](https://arxiv.org/abs/2305.05227) - 2025
- [Towards privacy-preserving conversation analysis in everyday life: Exploring the privacy-utility trade-off](https://www.sciencedirect.com/science/article/pii/S0885230825000488)  - CSL 2026
- [Noise robust automatic speech recognition: review and analysis](https://link.springer.com/content/pdf/10.1007/s10772-023-10033-0.pdf) - International Journal of Speech Technology 2023
---

## 🧠 Key Papers

Foundational and recent works in privacy-preserving speech technology.

- [Paper Title](link) — brief description  
- [Paper Title](link) — brief description  
- [Paper Title](link) — brief description  

---

## 📊 Datasets

Datasets used for speech privacy, speaker modeling, and robustness evaluation.

- [Dataset Name](link) — brief description  
- [Dataset Name](link) — brief description  
- [Dataset Name](link) — brief description  

---

## 🧩 Sub-topics

### 🎤 Speaker Representation

Techniques for encoding speaker identity (e.g., x-vectors, ECAPA-TDNN).

- [Overview of Speaker Modeling and Its Applications: From the Lens of Deep Speaker Representation Learning](https://arxiv.org/abs/2407.15188) - 2024  
- [Speaker Representation Learning: Theories, Applications and Practice](https://wsstriving.github.io/talk/2023-icassp-wespeaker/) - 2023  

---

### 🔀 Speech Representation Disentanglement

Methods for separating speaker identity from linguistic and acoustic factors.

- [Disentangling Voice and Content with Self-Supervision for Speaker Recognition](https://proceedings.neurips.cc/paper_files/paper/2023/file/9d276b0a087efdd2404f3295b26c24c1-Paper-Conference.pdf) - NeurIPS 2023 
- [Disentangling Speaker and Content in Pre-trained Speech Models with Latent Diffusion for Robust Speaker Verification](https://www.isca-archive.org/interspeech_2025/li25z_interspeech.pdf) — Interspeech 2025  
- [Disentangling Speech Representations Learning With Latent Diffusion for Speaker Verification](https://ieeexplore.ieee.org/document/11164168) - TASLP 2025
- [Can Diffusion Models Disentangle? A Theoretical Perspective](https://proceedings.neurips.cc/paper_files/paper/2025/file/00bd71667563acfea40499ad76229894-Paper-Conference.pdf) - NeurIPS 2025
- [Explainable Disentanglement on Discrete Speech Representations for Noise-Robust ASR](https://ieeexplore.ieee.org/abstract/document/11249036) - APSIPA 2025
---

### 🙈 Speaker Anonymization

Approaches to remove or obfuscate speaker identity while preserving speech utility.

- [Paper / Method](link) — description  
- [Paper / Method](link) — description  

---

### 🌍 Physical Environmental Factors

Research on privacy-preserving speech systems under realistic environmental conditions.

#### 🔊 Noise

- [Long-term conversation analysis: privacy-utility trade-off under noise and reverberation](https://ieeexplore.ieee.org/document/10694640) - IWAENC 2024
- [Disentangling speech from surroundings with neural embeddings](https://ieeexplore.ieee.org/abstract/document/10096435) - ICASSP 2023
- [De'hubert: disentangling noise in a self-supervised model for robust speech recognition](https://ieeexplore.ieee.org/abstract/document/10096603) - ICASSP 2023
- Additive noise
- Non-stationary noise
- Real-world acoustic noise

#### 🗣️ Multiple Speakers

- Overlapping speech
- Speaker mixtures
- Cocktail party scenarios

#### 🎙️ Multiple Devices

- Device mismatch
- Microphone heterogeneity
- Cross-device generalization

---

### 🌊 Diffusion Models

Generative models for speech synthesis, transformation, and anonymization.

- [Paper / Method](link) — description  
- [Paper / Method](link) — description  

---

### 🌐 Distributed Learning

Federated and decentralized learning methods for privacy-preserving speech systems.

- [Paper / Method](link) — description  
- [Paper / Method](link) — description  

---

## 🏁 Benchmarks

Evaluation frameworks and benchmark tasks for speech privacy and robustness.

- [Benchmark Name](link) — description  
- [Benchmark Name](link) — description  
- [Benchmark Name](link) — description  

---

## 🛠 Open-source Toolkits

Libraries and toolkits for building and evaluating privacy-preserving speech systems.

- [Toolkit Name](link) — description  
- [Toolkit Name](link) — description  
- [Toolkit Name](link) — description  

---

## 🏆 Challenges & Competitions

Community challenges driving progress in speech privacy research.

- [Challenge Name](link) — year — description  
- [Challenge Name](link) — year — description  
- [Challenge Name](link) — year — description  

---

## 🤝 Contribution

Contributions are welcome!  
If you have relevant papers, datasets, or tools, feel free to open a pull request or issue.

Please ensure resources are relevant to:
- Privacy-preserving speech processing
- Robust speech under noise, multi-speaker, or multi-device conditions

---

## 📄 License

This repository is intended for research and educational purposes only.

---

⭐ If you find this useful, consider giving it a star!